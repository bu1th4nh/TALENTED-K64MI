...
for epoch in range(len(p_list)):
    ...
    column_names = Ariel
    for i in range(10): df_dtw_inv[i][i] = 1
    df_dtw_inv = pd.DataFrame(df_dtw_inv, columns=column_names, index=column_names)
    plt.figure()
    sns.heatmap(abs(df_dtw_inv), cmap='Reds', annot=True, fmt=".3f", annot_kws={"size": 8})
