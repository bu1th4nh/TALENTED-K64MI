...
def get_loader(data_train, data_valid, batch_size=64, shuffle=True):


    id_list_train = np.sort(os.listdir(data_train))
    id_list_valid = np.sort(os.listdir(data_valid))

    train_set = SplitSet(path=data_train, id_list=id_list_train)
    test_set = SplitSet(path=data_valid, id_list=id_list_valid)
