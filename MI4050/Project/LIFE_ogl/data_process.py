...
out = pd.read_csv('data/raw/Outcomes-a.txt').set_index('RecordID')['In-hospital_death']
...
def parse_rec(values, masks, dir_):
...
def parse_id(id_):
    data = pd.read_csv('data/raw/{}.txt'.format(id_))
    ...
    values = (np.array(evals) - mean) / std
    ...
    rec['forward'] = parse_rec(values, masks, dir_='forward')
    rec['backward'] = parse_rec(values[::-1], masks[::-1], dir_='backward')

    rec = json.dumps(rec)

    save_file = os.path.join(save_path, id_)
    with open(save_file,'w') as f:
        f.write(rec)

for i in tqdm(range(len(patient_ids)), desc='Processing patients'):
    id_ = patient_ids[i]
    # print('Processing patient {}'.format(id_))
    # print('Processing {} of {}'.format(i, len(patient_ids)))
